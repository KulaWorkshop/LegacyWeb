...
import * as LevelManager from "./LevelManager.js?v=2";
...
UI.blockselects.forEach(
  (element) =>
    (element.onclick = () => {
      UI.blockselects.forEach(
        (element) => (element.parentElement.childNodes[1].classList = "")
      );
      element.parentElement.childNodes[1].classList = "selected";
      UI.UpdateMenuDisplay(control.object, currentWorld);
    })
);
...
UI.edittransporterconnection.onclick = () => {
  ...
  UI.blockselects.forEach((element, index) => {
    if (element.parentElement.childNodes[1].classList.contains("selected"))
      sideIndex = index;
  });
